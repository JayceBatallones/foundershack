...
/**
 * Handles the POST request to check the correct answers and update the user's progress.
 *
 * @param {Request} req - The request object containing the user data and their progress details.
...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      currentTopicIndex,
    } = checkCorrectAnswersSchema.parse(body);
    ...
    if (type == "skills") {
      answerList = await prisma.statistics
        .findMany({
          where: {
